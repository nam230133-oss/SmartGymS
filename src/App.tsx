...
export default function App() {
  ...
  const handleSubmittingServiceFeedback = (e: React.FormEvent) => {
    ...
    if (!serviceFeedbackText.trim()) {
      showToast(t("Vui lòng nhập nội dung đánh giá!", "Please write down your feedback!", "请输入评价内容！"));
      return;
    }
    ...
    setTimeout(() => {
      ...
      setServiceRating(5);
      showToast(t("Cảm ơn bạn đã gửi đánh giá dịch vụ!", "Thank you for submitting your feedback!", "感谢您提交服务评价！"));
    }, 1000);
  };
  ...
  const handleUpdatePassword = (e: React.FormEvent) => {
    e.preventDefault();
    if (!secOldPassword) {
      showToast(t("Vui lòng nhập mật khẩu hiện tại!", "Please enter your current password!", "请输入当前密码！"));
      return;
    }
    if (secOldPassword !== session.password) {
      showToast(t("Mật khẩu hiện tại không chính xác!", "Incorrect current password!", "当前密码不正确！"));
      return;
    }
    if (!secNewPassword) {
      showToast(t("Vui lòng nhập mật khẩu mới!", "Please enter a new password!", "请输入新密码！"));
      return;
    }
    if (secNewPassword.length < 6) {
      showToast(t("Mật khẩu mới tối thiểu 6 ký tự!", "New password must be at least 6 characters!", "新密码长度至少为 6 个字符！"));
      return;
    }
    if (secNewPassword !== secConfirmPassword) {
      showToast(t("Mật khẩu xác nhận không trùng khớp!", "Confirm password does not match!", "确认密码不一致！"));
      return;
    }
    ...
    setTimeout(() => {
      ...
      showToast(t("Đổi mật khẩu thành công!", "Password updated successfully!", "密码修改成功！"));
  ...
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    if (!loginPhone) {
      showToast(t("Vui lòng nhập Số điện thoại!", "Please enter Phone number!", "请输入手机号码！"));
      return;
    }
    if (!loginPass) {
      showToast(t("Vui lòng nhập Mật khẩu!", "Please enter Password!", "请输入密码！"));
      return;
    }
    ...
    if (registeredUser) {
      ...
      if (registeredUser.password === loginPass) {
        ...
        setCurrentScreen("HOME");
        showToast(t("Đăng nhập thành công!", "Login successfully!", "登录成功！"));
      } else {
        showToast(
          t(
      ...
      setCurrentScreen("HOME");
      showToast(t("Đăng nhập thành công với tài khoản Demo!", "Demo login successfully!", "演示账号登录成功！"));
      ...
      setCurrentScreen("HOME");
      showToast(t("Đăng nhập thành công (Khách vãng lai)!", "Logged in successfully as Guest!", "以访客身份登录成功！"));
    } else {
      showToast(
        t(
          "Số điện thoại hoặc Mật khẩu chưa chính xác! (Nhập 'demo' để vào nhanh)",
          "Incorrect Phone or Password! (Enter 'demo' to skip)",
          "手机号或密码不正确！（输入 'demo' 快速体验）"
        )
      );
    }
  };
  ...
  const handleRegisterStep1Submit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!regName.trim()) {
      showToast(t("Vui lòng nhập Họ và Tên!", "Please enter Full Name!", "请输入姓名！"));
      return;
    }
    if (!regPhone || regPhone.length < 8) {
      showToast(t("Số điện thoại đăng ký không hợp lệ!", "Invalid Phone Number!", "注册手机号码无效！"));
      return;
    }
    ...
    showToast(t("Đã gửi mã đăng ký OTP!", "OTP registration code sent!", "验证码已发送！"));
  };

  const handleOTPAutoFill = () => {
    setRegOTP(["1", "3", "2", "9"]);
    setShowOTPBubble(false);
    showToast(t("Đã tự động điền mã OTP: 1329!", "Autofilled OTP: 1329!", "已自动填充验证码：1329！"));
  };
  ...
  const handleOTPSubmit = () => {
    ...
    if (fullOtpString === "1329" || fullOtpString.length === 4) {
      ...
      showToast(t("Xác thực OTP thành công!", "OTP Verified successfully!", "验证码验证成功！"));
    } else {
      showToast(t("Mã xác thực không đúng! Hãy nhập 1329 hoặc bấm vào thông báo.", "Incorrect OTP! Try 1329.", "验证码不正确！请输入 1329 或点击顶部通知。"));
    }
  };
  ...
  const handleRegisterStep3Submit = (e: React.FormEvent) => {
    e.preventDefault();
    if (regPass.length < 6) {
      showToast(t("Mật khẩu tối thiểu 6 ký tự!", "Password must be at least 6 characters!", "密码长度至少为 6 个字符！"));
      return;
    }
    if (regPass !== regConfirmPass) {
      showToast(t("Mật khẩu xác nhận không khớp!", "Confirm password doesn't match!", "确认密码不匹配！"));
      return;
    }
    ...
    setCurrentScreen("HOME");
    showToast(t("Thiết lập mật khẩu thành công! Chào mừng hội viên mới!", "Password set successfully! Welcome!", "密码设置成功！欢迎新会员！"));
  };
  ...
  const handleLogoutAction = () => {
    ...
    setLoginPhone("");
    setLoginPass("");
    showToast(t("Đã đăng xuất tài khoản!", "Sign out successfully!", "已成功退出登录！"));
  };
  ...
  const handleSaveProfile = (e: React.FormEvent) => {
    e.preventDefault();
    if (!editName.trim()) {
      showToast(t("Họ và tên không được để trống!", "Full name cannot be empty!", "姓名不能为空！"));
      return;
    }
    if (!editAddress.trim()) {
      showToast(t("Địa chỉ không được để trống!", "Address cannot be empty!", "地址不能为空！"));
      return;
    }
    if (!editGender.trim()) {
      showToast(t("Vui lòng chọn giới tính!", "Please select your gender!", "请选择性别！"));
      return;
    }
    if (!editBirthDate.trim()) {
      showToast(t("Ngày sinh không được để trống!", "Birth date cannot be empty!", "出生日期不能为空！"));
      return;
    }
    if (editPassword && editPassword.length < 6) {
      showToast(t("Mật khẩu tối thiểu phải từ 6 ký tự!", "Password must be at least 6 characters!", "密码长度至少为 6 个字符！"));
      return;
    }
    ...
    setTimeout(() => {
      ...
      showToast(t("Cập nhật hồ sơ cá nhân thành công!", "Profile updated successfully!", "个人资料更新成功！"));
    }, 600);
  };
  ...
  const incrementWorkouts = () => {
    setSession(prev => {
      ...
      showToast(t(`Tuyệt vời! Ghi nhận buổi tập thành công: ${newVal}`, `Great! Workout session recorded: ${newVal}`, `太棒了！训练打卡成功：第 ${newVal} 次`));
      return {
        ...prev,
  ...
  const markAllNotificationsAsRead = () => {
    ...
    showToast(t("Đã đọc tất cả thông báo!", "Marked all as read!", "已全部标记为已读！"));
  };
  ...
  const clearAllNotifications = () => {
    setNotifications([]);
    showToast(t("Đã xóa toàn bộ thông báo!", "Cleared all notifications!", "已清空所有通知！"));
  };
                  ...
                  <div className="absolute left-3.5 right-3.5 top-[99px] bg-slate-900/98 border border-slate-800 rounded-3xl shadow-[0_15px_45px_0_rgba(0,0,0,0.9)] z-[150] p-3.5 backdrop-blur-xl animate-fade-in flex flex-col gap-3 text-left">
                    ...
                    <div className="flex items-center justify-between pb-1.5 border-b border-slate-800/60 shrink-0">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-[11px] font-bold text-white uppercase tracking-wider">
                          {t("THÔNG BÁO & TIN NHẮN", "NOTIFICATIONS & CHAT", "通知与消息")}
                        </span>
                      </div>
                        ...
                        {notificationTab === "message" && notifications.some(n => n.type === "message" && !n.isRead) && (
                          <button
                            onClick={() => {
                              ...
                              showToast(t("Đã đọc tất cả tin nhắn", "Marked all messages as read", "已将所有消息标记为已读"));
                            }}
                            type="button"
                            className="text-[9px] text-[#D2FF00] hover:underline font-medium cursor-pointer bg-transparent border-0"
                          >
                    ...
                    <div className="flex bg-zinc-950 p-[3px] rounded-xl border border-zinc-900/80 shrink-0 select-none">
                      ...
                      >
                        ...
                        <span>{t("Thông báo chung", "System Notifs", "系统通知")}</span>
                      ...
                      >
                        ...
                        <span>{t("Tin nhắn đến", "Support Inbox", "客服收件箱")}</span>
                        {notifications.filter(n => n.type === "message" && !n.isRead).length > 0 && (
                    ...
                    <div className="max-h-[220px] overflow-y-auto flex flex-col gap-2 pr-0.5 custom-scrollbar">
                      {notificationTab === "notif" && (
                        notifications.filter(n => n.type !== "message").length === 0 ? (
                          <div className="text-center py-8 text-zinc-500 text-[10px] font-sans">
                            {t("Không có thông báo hệ thống nào", "No system notifications", "暂无系统通知")}
                          </div>
                        ) : (
                          notifications.filter(n => n.type !== "message").map((n) => (
                            ...
                            >
                              ...
                              <button
                                ...
                                title={t("Xóa thông báo", "Delete notification", "删除通知")}
                              >
                      ...
                      {notificationTab === "message" && (
                        notifications.filter(n => n.type === "message").length === 0 ? (
                          <div className="text-center py-8 text-zinc-500 text-[10px] font-sans">
                            {t("Không có tin nhắn mới nào", "No new support messages", "暂无客服消息")}
                          </div>
                        ) : (
                          notifications.filter(n => n.type === "message").map((n) => (
                            <div
                  ...
                  >
                    <div className="flex justify-between items-center mb-1">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] uppercase font-bold text-zinc-400 font-mono tracking-widest">{t("TIN NHẮN (SMS)", "MESSAGE (SMS)", "短信 (SMS)")}</span>
                      </div>
                      <span className="text-[9px] text-[#D2FF00] font-mono font-bold animate-pulse">{t("BÂY GIỜ", "NOW", "刚才")}</span>
                    </div>
                    <p className="text-[11px] font-sans text-zinc-200 mt-1.5 leading-relaxed">
                      <span>{t("Mã OTP kích hoạt dịch vụ FITGYM của bạn là: ", "Your FITGYM OTP code is: ", "您的 FITGYM 验证码为: ")}<strong className="text-[#D2FF00] text-sm select-all font-black">1329</strong>{t(". Không tiết lộ cho bất kỳ ai.", ". Do not share it.", "。请勿泄露给他人。")}</span>
                    </p>
                    <div className="mt-2 text-center py-1.5 bg-[#D2FF00] hover:bg-[#c6ef00] transition rounded-xl text-black text-[10px] font-bold font-mono tracking-wider shadow-[0_2px_10px_rgba(210,255,0,0.2)]">
                      {t(`[ CHẠM ĐỂ TỰ ĐIỀN OTP ] (${bubbleCountdown}s)`, `[ TOUCH TO AUTO-FILL OTP ] (${bubbleCountdown}s)`, `[ 点击自动填充验证码 ] (${bubbleCountdown}s)`)}
                    </div>
                  </div>
                )}
                    ...
                    <div className="flex-1 flex flex-col justify-between px-6 pt-3 h-full animate-fade-in relative overflow-hidden bg-black">
                      ...
                      <div className="flex flex-col items-center justify-center pt-5 pb-3 relative z-10">
                        ...
                        <span className="text-[8px] text-zinc-400 font-mono uppercase tracking-widest mt-0.5 font-bold">
                          {t("ĐĂNG KÝ HỘI VIÊN // OTP", "SIGN UP MEMBER // OTP", "注册会员 // 验证码")}
                        </span>
                      </div>
                      ...
                      <form onSubmit={handleRegisterStep1Submit} className="flex flex-col gap-3 mt-4 relative z-10">
                        ...
                        <div className="flex flex-col gap-1 text-left">
                          <span className="text-[9px] text-zinc-400 font-mono tracking-wide font-bold">
                            {t("HỌ VÀ TÊN HỘI VIÊN", "MEMBER FULL NAME", "会员姓名")}
                          </span>
                          <input
                            type="text"
                            value={regName}
                            onChange={(e) => setRegName(e.target.value)}
                            placeholder={t("VD: NGUYỄN VĂN A", "E.G.: JOHN DOE", "例如：张三")}
                        ...
                        <div className="flex flex-col gap-1 text-left">
                          <span className="text-[9px] text-zinc-400 font-mono tracking-wide font-bold">
                            {t("SỐ ĐIỆN THOẠI ĐĂNG KÝ", "REGISTERED PHONE NUMBER", "注册手机号码")}
                          </span>
                          <input
                            type="text"
                            value={regPhone}
                            onChange={(e) => setRegPhone(e.target.value)}
                            placeholder={t("Nhập 10 chữ số (VD: 0987654321)", "Enter 10 digits (E.g.: 0987654321)", "输入10位数字（例如：0987654321）")}
                        ...
                        >
                          <span>{t("TIẾP TỤC & NHẬN OTP", "CONTINUE & GET OTP", "继续并获取验证码")}</span>
                    ...
                    <div className="flex-1 flex flex-col justify-between px-6 pt-3 h-full animate-fade-in relative overflow-hidden bg-black">
                      ...
                      <div className="bg-black/60 p-3 rounded-2xl border border-zinc-850 flex flex-col gap-1.5 relative z-10 shadow-[inner_0_1px_3px_rgba(0,0,0,0.4)]">
                        <div className="flex justify-between items-center">
                          ...
                          <span className="text-[10px] text-[#D2FF00] font-mono uppercase font-bold">
                            {t("BƯỚC 2 / 3", "STEP 2 / 3", "步骤 2 / 3")}
                          </span>
                        </div>
                        <p className="text-[10px] text-zinc-350 leading-snug font-sans">
                          <span>
                            {t("Mã xác thực OTP đã được kích hoạt đến số ", "OTP code simulated active and sent to ", "验证码已成功发送至 ")}
                            <strong className="text-[#D2FF00] tracking-wider select-all">
                              {session.phoneNumber || "0012312312"}
                            </strong>
                            {t(". Vui lòng kiểm tra tin nhắn.", ".", "。请注意查收短信。")}
                          </span>
                        </p>
                      </div>
                      ...
                      <div className="my-auto py-3 text-center relative z-10">
                        <span className="text-[10px] text-zinc-400 font-mono tracking-widest block mb-2 font-bold">
                          {t("MÃ OTP (4 CHỮ SỐ)", "OTP CODE (4 DIGITS)", "验证码 (4位数)")}
                        </span>
                      ...
                      <div className="flex flex-col items-center gap-4 mt-4 pb-3 relative z-10">
                        ...
                        <button
                          ...
                          className="text-[10px] text-zinc-300 font-sans tracking-wide uppercase hover:text-white transition"
                        >
                          {t("ĐÃ CÓ TÀI KHOẢN?", "HAD ACCOUNT?", "已有账户？")}{" "}
                      ...
                      <form onSubmit={handleRegisterStep3Submit} className="flex flex-col gap-3 mt-4 text-left relative z-10">
                        ...
                        <div className="flex flex-col gap-0.5">
                          <span className="text-[9px] text-zinc-400 font-mono uppercase font-bold">
                            {t("TẠO MẬT KHẨU", "CREATE PASSWORD", "创建密码")}
                          </span>
                          <div className="relative">
                            <input
                        ...
                        <div className="flex flex-col gap-0.5">
                          <span className="text-[9px] text-zinc-400 font-mono uppercase font-bold">
                            {t("XÁC NHẬN MẬT KHẨU", "CONFIRM PASSWORD", "确认密码")}
                          </span>
                          <input
                            type="password"
                        ...
                        <div className="flex gap-3 mt-2">
                          ...
                          >
                            <span>{t("HOÀN TẤT", "COMPLETE", "完成")}</span>
                      ...
                      <div className="flex items-center justify-between mb-4 mt-2 shrink-0">
                        <div className="flex items-center gap-2">
                          ...
                          <h3 className="font-display italic text-xl font-extrabold text-white uppercase leading-none tracking-wider">
                            {t("HỒ SƠ CÁ NHÂN", "PERSONAL PROFILE", "个人资料")}
                          </h3>
                        </div>
                        ...
                        <>
                          ...
                          <div className="w-full bg-zinc-900/80 border border-zinc-800/80 rounded-2xl p-4.5 mb-5 shadow-2xl relative overflow-hidden backdrop-blur-md shrink-0">
                            ...
                            <div className="space-y-3.5 relative z-10 border-t border-zinc-800/60 pt-4.5">
                              ...
                              <div className="flex justify-between items-start gap-4">
                                <span className="text-[9px] font-mono text-slate-400 uppercase tracking-wider font-extrabold whitespace-nowrap pt-0.5">
                                  {t("SỐ ĐIỆN THOẠI:", "PHONE NUMBER:", "手机号码:")}
                                </span>
                              ...
                              <div className="flex justify-between items-start gap-4">
                                <span className="text-[9px] font-mono text-slate-400 uppercase tracking-wider font-extrabold whitespace-nowrap pt-0.5">
                                  {t("GIỚI TÍNH:", "GENDER:", "性别:")}
                                </span>
                                <span className="text-xs text-white font-semibold text-right">
                                  {session.gender || t("Nam", "Male", "男")}
                                </span>
                              </div>

                              <div className="flex justify-between items-start gap-4">
                                <span className="text-[9px] font-mono text-slate-400 uppercase tracking-wider font-extrabold whitespace-nowrap pt-0.5">
                                  {t("NGÀY SINH:", "DATE OF BIRTH:", "出生日期:")}
                                </span>
                          ...
                          <div className="flex-1 flex flex-col justify-end mb-4">
                            ...
                            >
                              ...
                              <span>{t("CHỈNH SỬA THÔNG TIN", "EDIT PROFILE INFO", "编辑资料")}</span>
                            </button>
                          </div>
                        </>
                      ) : (
                            ...
                            <div className="space-y-4">
                              ...
                              <div className="flex items-center gap-1.5 pb-1 border-b border-zinc-900">
                                ...
                                <h4 className="text-[10px] font-mono tracking-wider text-slate-400 uppercase font-black">
                                  {t("HỒ SƠ ĐANG CHỈNH SỬA", "EDIT PROFILE DATA", "编辑中的资料")}
                                </h4>
                              </div>
                              ...
                              <div className="space-y-1 text-left">
                                ...
                                <div className="relative">
                                  ...
                                  <input
                                    ...
                                    placeholder={t("Nhập họ và tên...", "Enter full name...", "输入姓名...")}
                              ...
                              <div className="space-y-1 text-left">
                                <label className="text-[9px] font-mono tracking-widest text-[#D2FF00] uppercase font-bold block flex items-center gap-1">
                                  <span>{t("SỐ ĐIỆN THOẠI HỘI VIÊN", "MEMBER PHONE NUMBER", "会员手机号码")}</span>
                                ...
                                <span className="text-[8px] text-zinc-550 font-mono block italic leading-snug">
                                  * {t("Số điện thoại đăng ký là cố định và không thay đổi.", "Registration phone number is fixed and locked.", "注册手机号码固定不可更改。")}
                                </span>
                              </div>
                              ...
                              <div className="space-y-1 text-left">
                                ...
                                <div className="relative">
                                  ...
                                  >
                                    <option value="Nam">{t("Nam", "Male", "男")}</option>
                                    <option value="Nữ">{t("Nữ", "Female", "女")}</option>
                                    <option value="Khác">{t("Khác", "Other", "其他")}</option>
                                  </select>
                              ...
                              <div className="space-y-1 text-left">
                                ...
                                <div className="relative">
                                  ...
                                  <input
                                    ...
                                    placeholder={t("Nhập địa chỉ nhà...", "Enter address...", "输入住址...")}
                                    className="w-full pl-10 pr-4 py-2.5 bg-zinc-900 border border-zinc-800 rounded-xl focus:border-[#D2FF00] focus:outline-none text-xs text-white placeholder-zinc-650 font-sans font-medium"
                                  />
                                </div>
                              </div>
                              ...
                              >
                                {isSavingProfile ? (
                                  <>
                                    ...
                                    <span>{t("ĐANG LƯU THAY ĐỔI...", "SAVING CHANGES...", "正在保存更改...")}</span>
                                  </>
                                ) : (
                                  <>
                                    <CheckCircle className="size-4 text-black inline" />
                                    <span className="ml-1.5">{t("LƯU THAY ĐỔI", "SAVE UPDATE", "保存更改")}</span>
                                  </>
                                )}
                              </button>
                    ...
                    <div className="flex-1 flex flex-col justify-start px-6 pt-1 h-full animate-fade-in text-left">
                      ...
                      <div className="flex items-center justify-between mb-4 mt-2 shrink-0">
                        <div className="flex items-center gap-2">
                          ...
                          <h3 className="font-display italic text-xl font-extrabold text-white uppercase leading-none tracking-wider">
                            {t("LỊCH SỬ THANH TOÁN", "PAYMENT HISTORY", "支付历史")}
                          </h3>
                        </div>
                      </div>
                      ...
                      <div className="w-full bg-zinc-900/60 border border-zinc-850 rounded-2xl p-4.5 mb-4 shadow-xl relative overflow-hidden backdrop-blur-md shrink-0">
                        ...
                        <span className="text-[9px] font-mono text-slate-400 uppercase tracking-widest font-bold block mb-1">
                          {t("TỔNG SỐ GIAO DỊCH", "TOTAL TRANSACTIONS", "交易总数")}
                        </span>
                        ...
                        {paymentHistory.length === 0 ? (
                          <div className="flex flex-col items-center justify-center py-12 text-center bg-zinc-900/30 border border-dashed border-zinc-800 rounded-2xl px-4">
                            ...
                            <p className="text-xs font-bold text-zinc-400 uppercase tracking-wider">
                              {t("Chưa có giao dịch", "No transactions", "暂无交易记录")}
                            </p>
                              ...
                              <div className="space-y-2 bg-zinc-950/20 p-2.5 rounded-xl border border-zinc-900">
                                ...
                                <div className="flex items-center justify-between text-[11px] py-1 border-b border-zinc-900/50">
                                  <div className="flex items-center gap-1.5 text-zinc-400 font-medium">
                                    ...
                                    <span>{t("Gói tập", "Package", "订购方案")}</span>
                                  </div>
                                ...
                                <div className="flex items-center justify-between text-[11px] py-1 border-b border-zinc-900/50">
                                  <div className="flex items-center gap-1.5 text-zinc-400 font-medium">
                                    ...
                                    <span>{t("Hạn sử dụng", "Expiration Date", "有效到期")}</span>
                                  </div>
                                  <span className="font-mono text-[10px] font-black text-[#D2FF00] tracking-wide bg-[#D2FF00]/5 px-2 py-0.5 rounded border border-[#D2FF00]/10">
                                    {tx.expiryDate || t("Chưa kích hoạt", "Inactive", "未激活")}
                                  </span>
                                </div>
                                ...
                                <div className="flex items-center justify-between text-[11px] py-1 border-b border-zinc-900/50">
                                  <div className="flex items-center gap-1.5 text-zinc-400 font-medium">
                                    ...
                                    <span>{t("Phương thức", "Payment Method", "支付方式")}</span>
                                  </div>
                    ...
                    <div className="flex-1 flex flex-col justify-start px-6 pt-1 h-full animate-fade-in text-left">
                      ...
                      <div className="flex items-center justify-between mb-4 mt-2 shrink-0">
                        <div className="flex items-center gap-2">
                          ...
                          <h3 className="font-display italic text-xl font-extrabold text-white uppercase leading-none tracking-wider">
                            {t("CÀI ĐẶT BẢO MẬT", "SECURITY SETTINGS", "安全设置")}
                          </h3>
                        </div>
                      </div>
                            ...
                            <form onSubmit={handleUpdatePassword} className="space-y-3">
                              ...
                              <div className="space-y-1">
                                <label className="text-[9px] font-mono text-zinc-400 uppercase tracking-wider font-extrabold block">
                                  {t("MẬT KHẨU HIỆN TẠI", "CURRENT PASSWORD", "当前密码")}
                                </label>
                                <div className="relative">
                                  <input
                                    ...
                                    placeholder={t("Nhập mật khẩu", "Enter password", "输入密码")}
                              ...
                              <div className="space-y-1">
                                <label className="text-[9px] font-mono text-zinc-400 uppercase tracking-wider font-extrabold block">
                                  {t("MẬT KHẨU MỚI (TỐI THIỂU 6 KÝ TỰ)", "NEW PASSWORD (MIN 6 CHARS)", "新密码（最少6位字符）")}
                                </label>
                                <div className="relative">
                                  <input
                                    ...
                                    placeholder={t("Nhập mật khẩu mới", "Enter new password", "输入新密码")}
                                    className="w-full pl-3.5 pr-10 py-2.5 bg-zinc-950/50 border border-zinc-800 rounded-xl focus:border-[#D2FF00] focus:ring-0 focus:outline-none text-xs text-white placeholder-zinc-650 font-mono outline-none animate-none"
                                  />
                                  <button
                                    type="button"
                              ...
                              <div className="space-y-1">
                                <label className="text-[9px] font-mono text-zinc-400 uppercase tracking-wider font-extrabold block">
                                  {t("XÁC NHẬN MẬT KHẨU MỚI", "CONFIRM NEW PASSWORD", "确认新密码")}
                                </label>
                                <div className="relative">
                                  <input
                                    type="password"
                                    ...
                                    placeholder={t("Xác nhận mật khẩu mới", "Confirm new password", "确认新密码")}
                              ...
                              >
                                {isUpdatingPassword ? (
                                  <>
                                    <span className="w-3.5 h-3.5 rounded-full border-2 border-black border-t-transparent animate-spin inline-block mr-1.5" />
                                    <span>{t("ĐANG CẬP NHẬT...", "SAVING CHANGES...", "正在保存更改...")}</span>
                                  </>
                                ) : (
                                  <>
                                    <Check className="size-3.5 text-black inline" />
                                    <span className="ml-1.5 font-bold">{t("CẬP NHẬT MẬT KHẨU", "UPDATE PASSWORD", "更新密码")}</span>
                                  </>
                                )}
                              </button>
                            </form>
                        ...
                        {securityActiveTab === "RATING" && (
                          <div className="w-full bg-zinc-900/55 border border-zinc-850 rounded-2xl p-4 shadow-xl backdrop-blur-md animate-fade-in space-y-3">
                            <form onSubmit={handleSubmittingServiceFeedback} className="space-y-3">
                              ...
                              <div className="space-y-1">
                                <label className="text-[9px] font-mono text-zinc-400 uppercase tracking-wider font-extrabold block">
                                  {t("MỤC ĐÓNG GÓP Ý KIẾN", "FEEDBACK CATEGORY", "反馈类别")}
                                </label>
                                <div className="grid grid-cols-2 gap-1.5">
                                  {[
                                    { id: "PT", vi: "HLV (PT)", en: "PTs", zh: "私人教练" },
                                    { id: "GYM", vi: "Thiết bị", en: "Gym Equipment", zh: "健身器材" },
                                    { id: "SER", vi: "Dịch vụ CSKH", en: "Customer Support", zh: "客户服务" },
                                    { id: "APP", vi: "Ứng dụng di động", en: "Mobile App", zh: "移动应用" }
                                  ].map((cat) => (
                                    <button
                                      key={cat.id}
                                      type="button"
                              ...
                              <div className="space-y-1">
                                <label className="text-[9px] font-mono text-zinc-400 uppercase tracking-wider font-extrabold block">
                                  {t("Ý KIẾN ĐÓNG GÓP & ĐÁNH GIÁ", "YOUR WRITTEN FEEDBACK", "您的意见与反馈")} *
                                </label>
                                <textarea
                                  value={serviceFeedbackText}
                                  onChange={(e) => setServiceFeedbackText(e.target.value)}
                                  placeholder={t("Chia sẻ trải nghiệm của bạn...", "Share your workout experience...", "分享您的健身体验...")}
                                  rows={2}
                              ...
                              >
                                {isSubmittingServiceFeedback ? (
                                  ...
                                  <>
                                    ...
                                    <span className="ml-1.5 font-bold">{t("GỬI ĐÁNH GIÁ", "SUBMIT FEEDBACK", "提交反馈")}</span>
                                  </>
                                )}
                              </button>
                            </form>
                            ...
                            <div className="space-y-2 pt-3.5 border-t border-zinc-850">
                              <span className="text-[9px] font-mono text-zinc-500 uppercase tracking-widest font-black block mb-2">
                                {t("LỊCH SỬ ĐÁNH GIÁ GẦN ĐÂY", "RECENT EVALUATIONS", "近期评价历史")}
                              </span>
                              ...
                              <div className="space-y-2 max-h-[160px] overflow-y-auto pr-0.5 custom-scrollbar">
                                {serviceReviews.length === 0 ? (
                                  <div className="text-center py-6 text-zinc-650 font-mono text-[10px]">
                                    {t("CHƯA CÓ ĐÁNH GIÁ NÀO", "NO RATINGS SUBMITTED YET", "暂无评价记录")}
                                  </div>
                                ) : (
                ...
                {showLogoutConfirm && (
                  <div className="absolute inset-0 bg-black/93 backdrop-blur-md z-[999] flex flex-col justify-center items-center p-6 text-center">
                    ...
                    <h3 className="font-display italic text-lg font-black text-white uppercase tracking-wider leading-none mb-2">
                      {t("XÁC NHẬN ĐĂNG XUẤT?", "CONFIRM SIGN OUT?", "确认退出登录？")}
                    </h3>
                    ...
                    <p className="text-zinc-400 text-xs mb-8 max-w-[270px] leading-relaxed font-sans">
                      {t(
                        ...
                        "您确定要退出 FIT GYM 应用吗？您的健身数据已安全同步。"
                      )}
                    </p>
                    ...
                    <div className="bg-zinc-950 border border-zinc-800 rounded-3xl w-full p-5 text-left relative animate-fade-in max-h-[85%] overflow-y-auto">
                      ...
                      <div className="mt-4">
                        <span className="text-[10px] text-zinc-500 font-mono block mb-2 uppercase tracking-wider font-bold">
                          {t("PHƯƠNG THỨC THANH TOÁN:", "PAYMENT METHOD:", "支付方式:")}
                        </span>
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <span className="text-[11px] font-medium font-sans">
                                  {t("Ví điện tử MoMo", "MoMo E-Wallet", "MoMo 电子钱包")}
                                </span>
                              </div>
                            </div>
                          ...
                          >
                            <div className="flex items-center gap-3">
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <span className="text-[11px] font-medium font-sans">
                                  {t("Ví điện tử ZaloPay", "ZaloPay E-Wallet", "ZaloPay 电子钱包")}
                                </span>
                              </div>
                            </div>
                            <Smartphone className="size-3.5 opacity-60" />
                          ...
                          >
                            <div className="flex items-center gap-3">
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <span className="text-[11px] font-medium font-sans">
                                  {t("Thanh toán bằng Ngân hàng", "Bank Transfer / ATM", "银行转账 / ATM")}
                                </span>
                              </div>
                            </div>
